...
Apply the smooth EM algorithm to infer the latent orderings:
...
All the cell-type factors are assigned to the first latent ordering, and (almost) all the batch-type factors are assigned to the second latent ordering, except for the 14th factor, which is assigned to the first latent ordering.


Take a look at each inferred function in the first latent ordering:

```{r}
all_coordinates_1 <- permut_cols[resPancreas$J1]
par(mfrow = c(2, 4))
for (i in 1:8) {
  index <- order(all_coordinates_1)[i]
  mean_vec <- sapply(resPancreas$result1$params$mu, function(mu_k) mu_k[[index]])
  plot((1:100)/100, mean_vec, type = 'o',
       cex = 0.5,
       main = paste("Factor", all_coordinates_1[index]), 
       xlab = "discretized t1", ylab = paste("mu"))
}
par(mfrow = c(1, 1))
```
...
Take a look at each inferred function in the second latent ordering:

```{r}
all_coordinates_2 <- permut_cols[resPancreas$J2]
par(mfrow = c(2, 4))
for (i in 1:length(resPancreas$result2$params$mu[[1]])) {
  index <- order(all_coordinates_2)[i]
  mean_vec <- sapply(resPancreas$result2$params$mu, function(mu_k) mu_k[[index]])
  plot((1:100)/100, mean_vec, type = 'o',
       cex = 0.5,
       main = paste("Factor", all_coordinates_2[index]), 
       xlab = "discretized t2", ylab = paste("mu"))
}
par(mfrow = c(1, 1))
```
