...
```{r}
set.seed(123)
Q_prior_RW1 <- make_random_walk_precision(K=100, d=ncol(Loadings), lambda = 50, q=1, ridge = 0)
# Random initialization
init_params <- make_default_init(Loadings, K=100)
# # Initalize based on the tSNE metric
# init_params <- make_init(X = Loadings, ordering_vec = tsne_metric, K = 100)
result_RW1 <- EM_algorithm(
  data = Loadings,
  Q_prior = Q_prior_RW1,
  init_params = init_params,
  rank_deficiency = ncol(Loadings),
  max_iter = 100,
  modelName = "EEI",
  tol = 1e-3,
...
```{r}
set.seed(1234)
Q_prior_RW2 <- make_random_walk_precision(K=100, d=ncol(Loadings), lambda = 500, q=2, ridge = 0)
# Random initialization
init_params <- make_default_init(Loadings, K=100)
