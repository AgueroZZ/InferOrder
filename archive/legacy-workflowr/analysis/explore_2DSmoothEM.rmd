...
cluster_assign <- max.col(result_RW1$gamma)   
...
ggplot(cnt_full, aes(x = j / K2, y = i / K1, fill = n_obs)) +
  ...
  labs(
    x     = "latent j / K2",
    y     = "latent i / K1",
    title = "Observations per Latent Grid Cell (RW1)"
  ) +
  coord_equal() +
  theme_minimal()
```
...
# 1) Hard‐assign each observation to a latent grid index (1 … K1*K2)
cluster_assign <- max.col(result_RW2$gamma)   
...
ggplot(cnt_full, aes(x = j / K2, y = i / K1, fill = n_obs)) +
  ...
  labs(
    x     = "latent j / K2",
    y     = "latent i / K1",
    title = "Observations per Latent Grid Cell (RW2)"
  ) +
  coord_equal() +
  theme_minimal()
```
